...
def Find_BacksplashGalaxies(halos, mtree, Rcut0=3.0, hosthaloid=128000000000001, show_plot=0):
    ...
    Dist0           = norm2(halos[0]['Xhalo'],halos[0]['Yhalo'],halos[0]['Zhalo'],Xhost0,Yhost0,Zhost0)/Rhost0
...
def Find_InfallData(halos, mtree, haloids, hosthaloid=[128000000000001]):
    ...
    while (hosthaloid > 0):
        ...
        if(len(pos_good_haloids) > 0):
            ...
            pos_good_halos    = Find_arraypositions(halos[isnap]['haloid'],good_haloids)                
            
            # store distance, too
            X                         = halos[isnap]['Xhalo'][pos_good_halos]
            Y                         = halos[isnap]['Yhalo'][pos_good_halos]
            Z                         = halos[isnap]['Zhalo'][pos_good_halos]
            D[pos_good_haloids,isnap] = norm2(X,Y,Z, Xhost,Yhost,Zhost)/Rhost
...
def Calc_Dist2Host(halos, haloids, hosthaloid=128000000000001):
    ...
    if(len(pos_good_haloids) > 0):
        ...
        pos_haloids  = Find_arraypositions(halos['haloid'],good_haloids)
        X            = halos['Xhalo'][pos_haloids]
        Y            = halos['Yhalo'][pos_haloids]
        Z            = halos['Zhalo'][pos_haloids]
        
        # store distance
        D[pos_good_haloids] = norm2(X,Y,Z, Xhost,Yhost,Zhost)/Rhost
...
def Calc_Dist2HostTree(halos, mtree, haloids, hosthaloid=128000000000001):
    ...
    while isnap < nsnaps:
        ...
        if(len(pos_good_haloids) > 0):
            ...
            # positions in halos[isnap] where our haloids are found
            pos_haloids  = Find_arraypositions(halos[isnap]['haloid'],good_haloids)
...
def Find_minD(halos, mtree, haloids, hosthaloid=128000000000001):
    ...
    for i in range(nhalos):
        pos           = Find_arraypositions(D[i,:], minD[i])
        minD_isnap[i] = pos[0]
...
#    nsnaps     = np.size(halos)
    ...
    pos_now           = Find_arraypositions(halos[0]['haloid'], haloids)
    ...
    for isnap in range(1,nsnaps):
        ...
        pos                   = Find_arraypositions(halos[isnap]['haloid'], progids)
...
def Find_zreds(snapidmap, snapids):
    ...
    pos_snapids = Find_arraypositions(snapidmap['snapid'],snapids)
...
def Plot_Positions3D(halos, haloids):
	...
	ihalos = Find_arraypositions(halos['haloid'], haloids)
...
def Extract_uniqueSTARDUSTids(file1, file2):
    ...
    if(nhalos1 != nhalos2):
        ...
        for ihalo in range(nhalos1):
            ...
            # no idea where Remove_ids() can be found anymore :-(
            uniqueids.append(np.array(Remove_ids(pids,pidsremove)))
...
def Clean_particlesSTARDUSTexcised(STARDUST, clusterid):
    ...
    duppids = Find_Duplicates(allpids)
        
    # if there are duplicates, remove them from 'clusterid'
    if(len(duppids)>0):
        tag_bad  = Find_arraypositions(duppids, clusterpids)
...
#================================================================================
# find (and return) duplicates in an array (easier way!)
#================================================================================
