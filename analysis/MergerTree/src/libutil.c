...
// ALL THE MergerTree specific ROUTINES
void dump_defines()
{
  fprintf(stderr,"version = %f\n",mtree_version);
  
#ifdef USE_PTYPE
  fprintf(stderr,"USE_PTYPE\n");
#endif
  
#ifdef MTREE_BOTH_WAYS
  fprintf(stderr,"MTREE_BOTH_WAYS\n");
#endif
  
#ifdef USE_PIDMAP
  fprintf(stderr,"USE_PIDMAP\n");
#endif
  
#ifdef SNAPSKIPPING
  fprintf(stderr,"SNAPSKIPPING\n");
#ifdef SNAPSKIPPING_UNCREDIBLEMASSRATIO
  fprintf(stderr,"    SNAPSKIPPING_UNCREDIBLEMASSRATIO: %d\n",SNAPSKIPPING_UNCREDIBLEMASSRATIO);
#endif
#ifdef SNAPSKIPPING_CONSIDERALLPROGENITORS
  fprintf(stderr,"    SNAPSKIPPING_CONSIDERALLPROGENITORS\n");
#endif
#endif // SNAPSKIPPING
  
#ifdef READ_MPARTICLES
  fprintf(stderr,"READ_MPARTICLES\n");
#ifdef NDIGITS
  fprintf(stderr,"    NDIGITS: %d\n",NDIGITS);
#endif
#endif // READ_MPARTICLES
  
#ifdef THERE_IS_NO_NHALOS_LINE
  fprintf(stderr,"THERE_IS_NO_NHALOS_LINE\n");
#endif
  
#ifdef READ_HALOIDS_FROM_FILE
  fprintf(stderr,"READ_HALOIDS_FROM_FILE\n");
#endif
  
#ifdef USE_LINENUMBER_AS_HALOID
  fprintf(stderr,"USE_LINENUMBER_AS_HALOID\n");
#endif
  
#ifdef EXCLUSIVE_PARTICLES
  fprintf(stderr,"EXCLUSIVE_PARTICLES\n");
#endif
  
#ifdef WITH_QSORT
  fprintf(stderr,"WITH_QSORT\n");
#endif
  ...
  fprintf(stderr,"DEBUG_SNAPSKIPPING\n");
#endif

#ifdef MTREEDEVEL
  fprintf(stderr,"MTREEDEVEL\n");
#endif

#ifdef WITH_OPENMP
  fprintf(stderr,"WITH_OPENMP\n");
#pragma omp parallel
  {
    int ithread, ncpus, nthreads;
...
char *xstrdup(const char *s)
{
	...
	length = strlen(s) + 1;
	dummy  = malloc(sizeof(char) * length);
...
// ALL THE NUMERICAL RECIPES ET AL. ROUTINES
...
double cubic(double a1, double a2, double a3, double a4)
{
  ...
  if (fabs(a1) < ZERO)
    {
      fprintf(stderr,"ERROR: Quadratic/linear equation passed to CUBIC.");
      return(0.0);
    }
